...
<a href="${(frappe.router && frappe.router._subpath_prefix) || ""}/desk/${frappe.router.slug(doctype)}/${frappe.utils.escape_html(doc.name)}">${frappe.utils.escape_html(doc.name)}</a>
				</div>
			</div>
		</div>`;
	}
